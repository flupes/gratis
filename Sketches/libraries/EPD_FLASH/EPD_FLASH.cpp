...
EPD_FLASH_Class::EPD_FLASH_Class(uint8_t chip_select_pin)
    : EPD_FLASH_CS(chip_select_pin) {}

// TODO: Why allow begin to change the chip_select_pin (except for this could
// make chip_select_pin constant)
...
void EPD_FLASH_Class::write(uint32_t address, const void *buffer,
                            uint16_t length) {
  this->wait_for_ready();
...
#if defined(__AVR__)
void EPD_FLASH_Class::write_from_progmem(uint32_t address,
                                         PROGMEM const void *buffer,
                                         uint16_t length) {
  this->wait_for_ready();
  ...
  SPI.transfer(address >> 8);
  SPI.transfer(address);
  for (PROGMEM const uint8_t *p = (PROGMEM const uint8_t *)buffer; length != 0;
       ++p, --length) {

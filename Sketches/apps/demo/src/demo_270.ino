...
#else
// Temperature is handled by LM75 over
// I2C and not an analog pin
#endif
#if EPD_PWM_REQUIRED
...
// }
  ...
  SerialUSB.println();
  SerialUSB.println();
  SerialUSB.println("Demo version: " DEMO_VERSION);
  SerialUSB.println("Display size: " MAKE_STRING(EPD_SIZE));
  SerialUSB.println("Film: V" MAKE_STRING(EPD_FILM_VERSION));
  SerialUSB.println("COG: G" MAKE_STRING(EPD_CHIP_VERSION));
  SerialUSB.println();

  SerialUSB.println("Image 1: " IMAGE_1_FILE);
  SerialUSB.println("Image 2: " IMAGE_2_FILE);
  SerialUSB.println();

  EPD_FLASH.begin(Pin_EPD_FLASH_CS);
  if (EPD_FLASH.available()) {
    SerialUSB.println("EPD FLASH chip detected OK");
  } else {
    uint8_t maufacturer;
    uint16_t device;
    EPD_FLASH.info(&maufacturer, &device);
    SerialUSB.print("unsupported EPD FLASH chip: MFG: 0x");
    SerialUSB.print(maufacturer, HEX);
    SerialUSB.print("  device: 0x");
    SerialUSB.print(device, HEX);
    SerialUSB.println();
  }
...
void loop() {
  ...
  switch (EPD.error()) {
    case EPD_OK:
      SerialUSB.println("EPD: ok");
      break;
    case EPD_UNSUPPORTED_COG:
      SerialUSB.println("EPD: unsuppported COG");
      break;
    case EPD_PANEL_BROKEN:
      SerialUSB.println("EPD: panel broken");
      break;
    case EPD_DC_FAILED:
      SerialUSB.println("EPD: DC failed");
      break;
  }

...
class EPD_GFX : public Adafruit_GFX {
 private:
  ...
  // FIXME: Have to change these constants to suit panel (128 x 96, 200 x 96,
  // 264 x 176)
 ...
 public:
  ...
  // constructor
  EPD_GFX(EPD_Class &epd, TemperatureSensor &ts)
      : Adafruit_GFX(this->pixel_width, this->pixel_height),
        EPD(epd),
        temperature_sensor(ts) {}

  void begin(void) {
    int temperature = this->temperature_sensor.read();

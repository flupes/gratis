...
class EPD_Class {
 ...
 public:
  ...
  const bool operator!(void)const { return EPD_OK != this->status; }

  EPD_error error(void) const { return this->status; }
  ...
  void frame_data_13(const uint8_t *image_data, EPD_stage stage,
                     bool read_progmem = true);
  ...
  void line(uint16_t line, const uint8_t *data, uint8_t fixed_value,
            bool read_progmem, EPD_stage stage = EPD_normal,
            uint8_t border_byte = EPD_BORDER_BYTE_NULL,
            bool set_voltage_limit = false);

  // inline static void attachInterrupt();
  // inline static void detachInterrupt();

  EPD_Class(EPD_size _size, uint8_t panel_on_pin, uint8_t border_pin,
            uint8_t discharge_pin, uint8_t reset_pin, uint8_t busy_pin,
